...
def create_one_phase_input_file(inputs, input_file_name):
    ...
    with open(f'{input_folder}{input_file_name}', 'w') as f:
        ...
        # Geometry size
        f.write(f'\t<size> <x> {x_size} </x> <y> {y_size} </y> <z> {z_size} </z> </size>\n')
        # Periodicity
        f.write(f'\t<per> <x> {periodic_x} </x> <y> {periodic_y} </y> <z> {periodic_z} </z> </per>\n')
        f.write('</geometry>\n\n')

        # Write i/o section
        f.write('<folder>\n')
        # Input folder name
        f.write(f'\t<in_f> {input_folder} </in_f>\n')
        # Output folder name
        f.write(f'\t<out_f> {output_folder} </out_f>\n')
        f.write('</folder>\n\n')
        ...
        f.write(f'\t<num> {num_geoms_or_sims} </num>\n')
        # Pressure
        f.write(f'\t<press> {pressure} </press>\n')
        # Max simulation iterations
        f.write(f'\t<iter> {max_iter} </iter>\n')
        # Convergence
        f.write(f'\t<conv> {convergence} </conv>\n')
        # Save vtks
        f.write(f'\t<vtk_out> {save_vtks} </vtk_out>\n')
        f.write('</simulations>')
...
def create_two_phase_input_file(inputs, input_file_name):
    ...
    with open(f'{input_folder}{input_file_name}', 'w') as f:
        ...
        f.write('<geometry>\n')
        # Geometry name
        f.write(f'\t<file_geom> {input_folder}{geom_name}.dat </file_geom>\n')
        # Geometry size
        f.write(f'\t<size> <x> {x_size} </x> <y> {y_size} </y> <z> {z_size} </z> </size>\n')
        # Periodicity
        f.write(f'\t<per>\n')
        f.write(f'\t\t<fluid1> <x> {periodic_x} </x> <y> {periodic_y} </y> <z> {periodic_z} </z> </fluid1>\n')
        f.write(f'\t\t<fluid2> <x> {periodic_x} </x> <y> {periodic_y} </y> <z> {periodic_z} </z> </fluid2>\n')
        f.write(f'\t</per>\n')
        f.write('</geometry>\n\n')
        ...
        f.write(f'\t<fluid1>\n')
        f.write(f'\t\t <x1> {fluid1_x1} </x1> <y1> {fluid1_y1} </y1> <z1> {fluid1_z1} </z1>\n')
        f.write(f'\t\t <x2> {fluid1_x2} </x2> <y2> {fluid1_y2} </y2> <z2> {fluid1_z2} </z2>\n')
        f.write(f'\t</fluid1>\n')
        f.write(f'\t<fluid2>\n')
        f.write(f'\t\t <x1> {fluid2_x1} </x1> <y1> {fluid2_y1} </y1> <z1> {fluid2_z1} </z1>\n')
        f.write(f'\t\t <x2> {fluid2_x2} </x2> <y2> {fluid2_y2} </y2> <z2> {fluid2_z2} </z2>\n')
        f.write(f'\t</fluid2>\n')
        f.write('</init>\n\n')
        ...
        f.write(f'\t<Gc> {Gc} </Gc>\n')
        f.write(f'\t<omega_f1> {omega_f1} </omega_f1>\n')
        f.write(f'\t<omega_f2> {omega_f2} </omega_f2>\n')
        f.write(f'\t<force_f1> {force_f1} </force_f1>\n')
        f.write(f'\t<force_f2> {force_f2} </force_f2>\n')
        f.write(f'\t<G_ads_f1_s1> {G_ads_f1_s1} </G_ads_f1_s1>\n')
        f.write(f'\t<G_ads_f1_s2> {G_ads_f1_s2} </G_ads_f1_s2>\n')
        f.write(f'\t<G_ads_f1_s3> {G_ads_f1_s3} </G_ads_f1_s3>\n')
        f.write(f'\t<G_ads_f1_s4> {G_ads_f1_s4} </G_ads_f1_s4>\n')

        f.write(f'\t<rho_f1> {rho_f1} </rho_f1>\n')
        f.write(f'\t<rho_f2> {rho_f2} </rho_f2>\n')

        f.write(f'\t<pressure_bc> {pressure_bc} </pressure_bc>\n')
        f.write(f'\t<rho_f1_i> {rho_f1} </rho_f1_i>\n')
        f.write(f'\t<rho_f2_i> {rho_f2} </rho_f2_i>\n')
        ...
        f.write(f'\t<save_it> {save_iter} </save_it>\n')
        f.write(f'\t<save_sim> {save_sim} </save_sim>\n')
        f.write(f'\t<convergence> {convergence} </convergence>\n')
        f.write(f'\t<it_max> {max_iter} </it_max>\n')
        f.write(f'\t<it_conv> {convergence_iter} </it_conv>\n')
        f.write(f'\t<it_gif> {gif_iter} </it_gif>\n')
        f.write(f'\t<rho_vtk> {rho_f2_vtk} </rho_vtk>\n')
        f.write(f'\t<it_vtk> {vtk_iter} </it_vtk>\n')
        f.write(f'\t<print_geom> {print_geom} </print_geom>\n')
        f.write(f'\t<print_stl> {print_stl} </print_stl>\n')
